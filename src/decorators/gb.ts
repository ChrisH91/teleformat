...
const decorator: IDecorator = {
  decorate(phoneNumber: string) {
    ...
    if (phoneNumber.substring(2, 4) == "02") {
      localNumber = `${phoneNumber.substring(3, 5)} ${phoneNumber.substring(5, 9)} ${phoneNumber.substring(9)}`;
    } else if (
      ...
      localNumber = `${phoneNumber.substring(3, 6)} ${phoneNumber.substring(6, 9)} ${phoneNumber.substring(9)}`;
    } else if (sixDigitAreaCodes.indexOf(phoneNumber.substring(2, 8)) != -1) {
      localNumber = `${phoneNumber.substring(2, 8)} ${phoneNumber.substring(8)}`;
    } else {
      localNumber = `${phoneNumber.substring(2, 7)} ${phoneNumber.substring(7)}`;
    }
